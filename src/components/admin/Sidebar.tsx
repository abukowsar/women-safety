...
interface SidebarProps {
  ...
  setActiveTab: (
    tab: "dashboard" | "cases" | "ai-insights" | "settings",
  ) => void;
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 shadow-xl`}
    >
      ...
      <div className="absolute bottom-0 w-full p-4 border-t border-slate-800">
        <button
          ...
          className="flex items-center space-x-2 text-slate-400 hover:text-white w-full px-4 py-2 hover:bg-slate-800 rounded-lg transition-colors"
        >
