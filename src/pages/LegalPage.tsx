...
const LegalPage: React.FC<LegalPageProps> = ({ currentLang }) => {
  ...
  useEffect(() => {
    const fetchLegal = async () => {
      try {
        const res = await contentService.getLegal();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        {legalData.map((item, index) => (
          ...
          >
            ...
            <span className="mt-4 inline-block bg-brand-100 text-brand-700 text-xs font-bold px-2 py-1 rounded">
              {item.category}
            </span>
          </div>
        ))}
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <ul className="space-y-4">
            <li className="flex items-start">
              <span className="bg-green-100 text-green-800 text-xs font-bold px-2 py-1 rounded mt-1 mr-3 min-w-fit">
                {t.pca.badge}
              </span>
        ...
        <div className="bg-brand-50 p-8 rounded-2xl shadow-sm border border-brand-100">
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="bg-white p-3 rounded-lg flex items-center shadow-sm">
              <div className="h-8 w-8 min-w-[2rem] rounded-full bg-slate-200 flex items-center justify-center font-bold mr-3 text-sm">
                1
              </div>
